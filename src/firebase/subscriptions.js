 ...
 */
export const subscribeToUser = async (subscriberId, targetUserId, targetData, subscriberData = {}) => {
  try {
    const subscriptionId = `${subscriberId}_${targetUserId}`;
    const subscriptionRef = doc(db, SUBSCRIPTIONS_COLLECTION, subscriptionId);
    
    await setDoc(subscriptionRef, {
      subscriberId: subscriberId,
      targetUserId: targetUserId,
      subscriberName: subscriberData.name || subscriberData.displayName || '',
...
export const unsubscribeFromUser = async (subscriberId, targetUserId) => {
  try {
    ...
    console.error('[Subscriptions] Unsubscribe error:', error.message);
    throw error;
  }
};
...
export const isSubscribed = async (subscriberId, targetUserId) => {
  try {
    ...
    console.error('[Subscriptions] Check subscription error:', error.message);
    return false;
  }
};
...
export const getSubscriptions = async (userId) => {
  try {
    ...
    console.error('[Subscriptions] Get subscriptions error:', error.message);
    return [];
  }
};
...
export const getSubscribers = async (userId) => {
  try {
    ...
    console.error('[Subscriptions] Get subscribers error:', error.message);
    return [];
  }
};
...
export const subscribeToSubscriptions = (userId, callback) => {
  try {
    ...
    console.error('[Subscriptions] Subscribe setup error:', error.message);
    return null;
  }
};
...
export const getSubscriberCount = async (userId) => {
  try {
    ...
    console.error('[Subscriptions] Get subscriber count error:', error.message);
    return 0;
  }
};

/**
 * Получить список подписчиков пользователя с их данными
 */
  ...
  try {
    ...
    snapshot.forEach((doc) => {
      ...
      subscribers.push({
        ...
        subscriberName: data.subscriberName || data.targetName || 'Unknown',
        subscriberEmail: data.subscriberEmail || data.targetEmail || '',
        subscriberAvatar: data.subscriberAvatar || data.targetAvatar || '',
        ...data
      });
    });
    
    console.log('[Subscriptions] Found subscribers:', subscribers.length, subscribers);
    return subscribers;
  } catch (error) {
    console.error('[Subscriptions] Get subscribers with user data error:', error.message);
    return [];
  }
};
